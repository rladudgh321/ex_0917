...
export async function uploadImageAPI({file, token}: { file: string, token: string }) {
  try {
    console.log('uploadImageAPI console', token);
    const headers = {
      Authorization:`Bearer ${token}`
    }

    const imagePaths = [] as Array<string>;
    // const formData = new FormData();
    // imagePaths.forEach((p) => {
    //   formData.append('file', p);
    // });

    const response = await axios.post('/post/upload', { file: formData, token }, { headers });
    console.log('uploadImageAPI', response.data);
    return response.data;
  } catch (err) {
    console.error(err);
  }
